...
- 全部在**临时目录和项目副本**里跑，不读写用户真实数据；副本里的配置会关掉提示音。
  （2026-09-24 修过一次漏洞：核心用例只换了"历史文件"，快照/配置/密库仍指向真实目录，
  一轮门禁把用户 8.5MB 的快照清空了。现在这几个路径全部指到临时目录，
  并且每轮跑完会自动核对真实数据文件没被改动——`.youboard.json` 除外，
  项目目录里正开着程序时它本来就会变。）
...
- 密库主密码（3.3.4）的硬约束：
  1. 设了主密码后，密库文件是 `{"format": "youboard-vault", ...}` 信封（PBKDF2 派生
     的密钥加密），**不再用 `youboard.key`**；`vault_files/` 里的图片也是同一把钥匙加密的。
  2. **锁定状态下 `VaultStore._save()` 必须返回 False**：锁定时内存里没有条目，
     真写下去等于把用户密库清空（门禁里有"锁着不许写 / 文件没被清空"的断言）。
  3. 主密码只在本机校验、不落盘；忘记了只能用「清空密库并重设」（`VaultStore.wipe()`）。
  4. 界面上的锁屏 / 自动锁定 / 关窗即锁在 `VaultDialog`（`VAULT_IDLE_LOCK_MS`）；
