...
### 安装版（推荐）
下载 `YouBoard_Setup_v1.3.0.exe`，双击安装，自动创建快捷方式和卸载程序。
...
- 🖼️ 自定义背景：设置 → 背景 → 上传自己的图片作为界面壁纸
  （推荐 1920×1080，支持 PNG/JPG/BMP，自动适配暗色/亮色遮罩）
- 🎨 全新应用图标 YouBoard.ico
- 彻底修复首次启动任务栏/托盘显示 Python 羽毛图标的问题
  （iconbitmap + iconphoto 双重设置，覆盖所有窗口）
- 安装包使用新图标，现代向导界面
- 图标资源统一为 YouBoard.ico（兼容旧版 You.ico 回退）
