...
- **v3.3.4** — 修复「更新失败会把主程序删掉」：回滚前先确认备份可用（没备份就保留现有主程序），新版确认启动后才删备份；密库新增**主密码**（单独派生密钥、打开需解锁、关窗自动锁定，闲置时间可选 1/5/15/30 分钟或**不自动锁定**）；AI 配置的「自定义」服务商不再丢地址/模型；卡片截图不再带周围背景，并改为按打开它的窗口居中（设置里开的卡片落在设置窗口正中，密库里的确认卡片落在密库窗口正中）
...
- 密库可以另设**主密码**：PBKDF2 派生一把独立密钥（与 `youboard.key` 无关），密库清单和里面的图片都按它加密，打开密库要解锁，闲置 5 分钟或关窗自动锁定；主密码只在本机校验、不落盘，忘了只能清空密库重设
- 说清楚边界：本地加密防的是**文件被单独拷走 / 云盘泄露 / 旁人翻程序**；它挡不住**本机上正在运行的恶意程序**（能在你输入主密码时截键盘那一类）
- AI 请求只包含你在列表里选中的那一条记录；API Key 加密保存、不写日志
- 手机传输只监听局域网、每次启动随机 token；云同步为端到端加密，云端只存密文

## 📜 更新日志

- **v3.3.4** — 更新失败不再删主程序；密库可设**主密码**（独立密钥、打开需解锁、闲置自动锁定时间可选 1/5/15/30 分钟或**不自动锁定**、关窗即锁）；AI 自定义服务商配置不再丢；卡片截图不带背景、按打开它的窗口居中
