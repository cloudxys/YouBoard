...
- **AI 自由对话** — 右键「AI 处理 ▸」→「和 AI 聊聊…」：选中的记录作为上下文，想问什么问什么，多轮问答 + 流式显示；只带最近 6 轮提问，token 不会越滚越多
- **AI 弹层导出 TXT** — 动作模式导出结果、对话模式导出整段对话（`你：… / AI：…`），与「文本」分类的导出同一套做法
- **服务商预设校正** — DeepSeek 改为官方文档的 `https://api.deepseek.com` + `deepseek-flash` / `deepseek-v4-pro`；通义 / 智谱 / OpenAI / Ollama 地址也逐个实测过（仍可手动改）
- **光标修复** — 小组件上缩放光标不再被"兜底刷新"清掉；设置里所有可点按钮（如「打开同步窗口」）都会稳定显示手型，不再"只有从下往上才有反应"
- **弹层与小组件** — 打开设置 / AI 配置 / 历史保留策略时小组件照常可点可复制，也不会被全屏遮罩盖住；卡片弹层不再始终置顶，切到别的程序时不会飘在别人窗口上；AI 对话输入问题后不会再直接显示「已停止生成」
- **设置窗口尺寸** — 默认 480×660（英文 600×660），限制在屏幕可用区内，最小可缩到 420×420
