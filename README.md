...
- **密库（顶栏挂锁按钮）** — 把密码、密钥、备注这类私密数据**由你主动放进去**：独立文件加密保存（`youboard_vault.json`，Fernet 加密 + 原子写入），支持搜索 / 新增 / 编辑 / 删除，「复制账号 / 复制密码」不会把内容记进剪贴板历史；密库也不参与手机传输、云同步和历史快照
- **修复「编辑标签」没有用** — 以前在输入框里打完标签**必须按回车**才算数，直接点「保存」那串文字会被丢掉（看起来就是改了没反应）；现在按回车和直接点保存都生效
