...
- **v3.3.4** — 修复「更新失败会把主程序删掉」：回滚前先确认备份可用（没备份就保留现有主程序），新版确认启动后才删备份；密库新增**主密码**（单独派生密钥、打开需解锁、关窗自动锁定，闲置时间**自己填秒或分钟**，也可以选**不自动锁定**）；AI 配置的「自定义」服务商不再丢地址/模型；卡片截图不再带周围背景，并改为按打开它的窗口居中（设置里开的卡片落在设置窗口正中，密库里的确认卡片落在密库窗口正中）
...
- **v3.3.4** — 更新失败不再删主程序；密库可设**主密码**（独立密钥、打开需解锁、闲置自动锁定时间**自己填秒或分钟**（也可选**不自动锁定**）、关窗即锁）；AI 自定义服务商配置不再丢；卡片截图不带背景、按打开它的窗口居中
