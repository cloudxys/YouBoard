...
- **密库** — 顶栏挂锁按钮：密码 / 密钥 / 备注等私密数据手动存进去，独立加密落盘，不进历史、不参与同步
...
- **密库（顶栏挂锁按钮）** — 把密码、密钥、备注这类私密数据**由你主动放进去**：独立文件加密保存（`youboard_vault.json`，Fernet 加密 + 原子写入），支持搜索 / 新增 / 编辑 / 删除，「复制账号 / 复制密码」不会把内容记进剪贴板历史；密库也不参与手机传输、云同步和历史快照。列表里**右键任意文本 / 网址记录 →「加入密库…」**也能直接存进去（密码字段预填那条内容，可再改）
