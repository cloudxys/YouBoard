...
- **标签 / 收藏（新增，纯本地）** — 任意一条记录都能打标签、加星收藏：右键菜单里选「加入收藏」，或点右侧预览顶上的收藏按钮；收藏过的记录在列表内容前会多一颗主题色五角星。搜索框里直接输标签名，也能找到打了这条标签的记录
- **筛选胶囊（收藏 / 标签）** — 标签页上方多出一排小胶囊：`收藏 N` 和 `#标签 N`（带数量，只在有收藏或标签时出现）。点一下只看这一类，再点一次取消；能跟关键词搜索、「文件」细分筛选叠加使用
- **编辑标签** — 右键「编辑标签…」给一条记录增删标签，右侧预览顶上也能进；选中多条时是「批量添加标签」（已经有的标签不会重复加，会显示成「已有」）
- **标记跟着记录走，不会丢** — 标签与收藏存在本机历史文件里，不上传、不联网；重复复制同一段内容、导入其它 YouBoard 数据、云同步合并时，标签和收藏都会被保留
- **可自定义快捷键多一个** — 设置 → 动作快捷键里新增「收藏 / 取消收藏」（默认 `Ctrl+D`），和其它动作一样能改
- 限制：单个标签最长 24 个字符，一条记录最多 20 个标签

> 从 v3.2.6 及更早版本升级，以下内容也一并包含。
...
### 安装版（推荐）
下载 `YouBoard_Setup_v3.2.7.exe`，双击安装，自动创建快捷方式和卸载程序。
...
- 🏷️ **标签 / 收藏（纯本地，零外部依赖）**
  - 任意记录可加星收藏、可打多个标签；列表内容前用主题色五角星标出收藏项
  - 标签页上方新增筛选胶囊 `收藏 N` / `#标签 N`，点击只看这一类，再点取消；与搜索、文件细分筛选可叠加
  - 右键菜单：「加入收藏 / 取消收藏」「编辑标签…」；多条选中时是批量添加标签
  - 搜索框输标签名即可命中对应记录；标签与收藏写入本机历史，重复复制 / 导入其它安装 / 云同步合并都不会丢
  - 新增可自定义快捷键「收藏 / 取消收藏」（默认 `Ctrl+D`）
  - 星标用矢量绘制，不依赖字体是否含 `★` 字形（缺字体的机器上不会掉成方块）
