...
- **标签 / 收藏（纯本地）** — 右键打标签、加星收藏；列表收藏行带主题色五角星，标签页上方多一排筛选胶囊（`收藏 N` / `#标签 N`，可与搜索、文件细分叠加）；搜索框输标签名也能命中
- **AI 就地处理** — 右键「AI 处理 ▸」总结 / 翻译 / 改写 / 提取要点 / 自定义提示（快捷键 `Ctrl+I`）；流式显示，结果可复制、另存为新条、替换本条（替换时标签与收藏保留）
- **设置页精简** — AI 服务收进卡片弹层（与「历史保留策略」同款），设置页只留一行摘要 +「配置」；可选择 DeepSeek / 通义千问 / 智谱 GLM / OpenAI / Ollama 本地 / 任意 OpenAI 兼容接口
- **打开设置时桌面小组件仍可用** — 设置与卡片弹层改为窗口级模态，不再禁用小组件
- **界面细节** — 文件分类胶囊在宽窗口下不再被摊开；AI 服务弹层中文标签不再被裁；去掉输入框与数字框的直角框
