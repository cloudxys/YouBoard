...
- **密库改成「移入 / 移出」，不再留一份在外面** — 以前「加入密库」后，剪贴板历史里还留着同一条（密钥等于没藏住）。现在保存进密库时会把它**从历史里移走**：记录本身不丢，连同**原来的时间戳、标签、收藏、置顶**一起存进密库；在密库里右键 →「移出密库…」就能放回去，并且**按当初的时间归位**，标签与收藏也一并还原
- **密库列表有了右键菜单** — 和主界面历史列表同一套：复制（内容 / 图片 / 文件）、导出为 .txt…、打开（查看器 / 定位）、复制路径、打开所在文件夹、编辑、重命名、删除；只有「加入密库」换成了「移出密库」

> 从 v3.3.1 及更早版本升级，以下内容也一并包含。
